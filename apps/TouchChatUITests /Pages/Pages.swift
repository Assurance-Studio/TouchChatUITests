...
class Pages {
    ...
    let wordPower25TouchScan = XCUIApplication().tables.staticTexts["WordPower25 SS"]
...
//        sleep(5)
    ...
    func openDifferentLanguagePageMega(languageVocab: String){
        app.buttons["Menu"].tap()
        app.buttons["Language & Region"].tap()
        let languageType = app.staticTexts[languageVocab]
    ...
    func vocabData(name: String, description: String) -> (vocabularyName: String, vocabularyDesc: String) {
        return (vocabularyName: name, vocabularyDesc: description)
    }
    ...
    func pressOnVocabIfVisible(){
        // Get cells containing the static texts
          let frenchCell = app.cells.containing(.staticText, identifier: "French").element
          let englishCell = app.cells.containing(.staticText, identifier: "English").element

          // Get the first image (indicator arrow) inside each cell
          ...
          if frenchIndicator.exists {
              ...
              if label == "expanded" || label.contains("down") || label == "▼" {
                  ...
                  print("French is not expanded → skipping tap")
              }
          }
          ...
          if englishIndicator.exists {
              ...
              if label == "expanded" || label.contains("right") || label == "►" {
                  ...
                  print("English is already expanded → skipping tap")
              }
          }
